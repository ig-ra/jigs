...
- **Stall-based per-angle cap** (parked in `plan.md` §P3b "Pending refinement") — replace flat
  ...
  class-generalization discipline, after data from real runs. **Both preconditions now exist:**
  the `CLASS:` line on every FOLD (SKILL.md invariant 6) makes "a NEW class this round" a recorded
  fact rather than a recollection, and the per-round FOLD/DISCUSS/DROP mix gives the stall signal.
  Needs data from 2–3 real runs before flipping the rule.
